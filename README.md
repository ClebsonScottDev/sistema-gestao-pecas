...
├── demo_dados.py            # popula dados de exemplo (não interativo)
├── tests/
│   └── test_sistema.py      # 19 testes automatizados (unittest)
├── docs/
│   └── documento_teorico.md # Parte 1 do entregável (análise e discussão)
├── documento_teorico.pdf    # Parte 1 em PDF
├── README.md
├── LICENSE
└── .gitignore
```

> Os arquivos `dados_producao.json`, `relatorio_final.txt/.json` e
> `producao.log` são gerados automaticamente em tempo de execução — por
> isso não aparecem no repositório (estão no `.gitignore`).
...
git clone https://github.com/ClebsonScottDev/sistema-gestao-pecas.git
